<template>
  <div class="purchaser_header">
    <div class="top">
      <van-nav-bar title="采购商注册" left-text="返回" left-arrow @click-left="goLogin()" />
    </div>
    ...
    >
      ...
      <el-form-item label="验证码：" prop="smsCode">
        <el-input v-model="purchaserHeader.smsCode" placeholder="验证码" @change="next"></el-input>
      </el-form-item>

      <el-form-item label="登陆密码：" prop="pwd">
        <el-input v-model="purchaserHeader.pwd" placeholder="请输入的密码" show-password></el-input>
      </el-form-item>
      <el-form-item label="确认密码：" prop="confirmPassWord">
        <el-input v-model="purchaserHeader.confirmPassWord" placeholder="请输入确认密码" show-password></el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        // ghlx: [{ required: true, message: "请选择采购类型", trigger: "blur" }]
      },
      show: true,
      count: "",
      timer: null,
  ...
  methods: {
    ...
    chooseItem(value) {
      this.purchaserHeader.ghlx = value.join(",");
    },
    submit() {
      if (this.purchaserHeader.pwd != this.purchaserHeader.confirmPassWord) {
        Notify({ type: "danger", message: "密码不一致" });
        return;
      }

      this.$refs["purchaserHeader"].validate(valid => {
        if (valid) {
          this.$axios
            .post(this.url + "/api/User/RegisterUser", {
              sjh: this.purchaserHeader.phoneNo,
              lx: 2,
              pwd: this.purchaserHeader.pwd,
              smsCode: this.purchaserHeader.smsCode,
              ghlx: this.purchaserHeader.ghlx
            })
            .then(res => {
              if (res.data.statusCode == 0) {
                localStorage.setItem(
                  "purchaserGhlx",
                  this.purchaserHeader.ghlx
                );
