        ...
        <div class="content" v-if="demandDatas.length > 0">
          <div class="content_item" v-for="(data,index) in demandDatas" :key="index">
            <el-row class="name">
              ...
              <el-col :span="14" style="height: 40px;line-height: 40px">
                <div class="item_layer">
                  <a @click="showSupplier(data.kzjjlistoid)">{{data.quotationName}}</a>
                </div>
              </el-col>
              <el-col
                :span="6"
            ...
            <van-row type="flex">
              ...
              <van-col span="16">
                <ul class="name_layer">
                  <li class="item_name" @click="viewDetails(data.order.kzxqorderoid)">
                    <a>{{data.order.spmc}}</a>
                  </li>
                ...
                <div class="price_section">订单数量：{{data.jjsl}}{{data.company}}</div>
                <div
                  class="pay_type"
...
export default {
      ...
      self.scroll.on("pullingUp", () => {
        ...
        if (!self.show) return;
          ...
          if (self.skipCount < self.lastPage) {
            ...
            self.$axios
              .post(self.url + "/api/Quotation/GetReachDemandAllPage", {
  ...
  methods: {
    getDemandDatas() {
      ...
      self.$axios
        .post(
          self.url + "/api/Quotation/GetReachDemandAllPage",
    ...
    //   Dialog.confirm({
    //     title: "请确认",
    //     message: "此意向未和供应商达成订单"
    //   })
    //     .then(() => {
    //       var token = localStorage.getItem("loginToken");
    //       axios.defaults.headers.common["Authorization"] = "Bearer " + token;
    //       self.$axios
    //         .get(self.url + "/api/Quotation/UnFinisQuotationOfBuyer?id=" + id)
    //         .then(res => {
      ...
      if (index == 1) {
        Dialog.confirm({
          ...
          .then(() => {
            ...
            axios.defaults.headers.common["Authorization"] = "Bearer " + token;
            self.$axios
              .get(self.url + "/api/Quotation/FinisQuotationOfBuyer?id=" + id)
              .then(res => {
                if (res.data.statusCode == 9) {
                  Notify({ type: "warning", message: res.data.resultMsg });
                }
    ...
    showSupplier(id) {
      ...
      localStorage.setItem("showOne", 2);
