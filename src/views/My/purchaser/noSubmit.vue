<template>
  <div class="no_confirmed">
    <div class="top">
      <van-nav-bar title="待提交的订货" left-text="返回" left-arrow @click-left="$router.push('/me')"></van-nav-bar>
    </div>
    <div class="wrapper">
            ...
            <van-row type="flex">
              ...
              <van-col span="10">
                ...
                <div v-if="data.productPublish.doctype == 1">
                  <!-- <div class="pay_type">最小起订量:{{data.productPublish.startqty}}</div> -->
                </div>
                <div v-if="data.productPublish.doctype == 2">
                  <!-- <div class="pay_type">每单最小数量：{{data.productPublish.startqty}}</div> -->
                  <div class="pay_type">成团数量：{{data.productPublish.groupbuyqty}}</div>
                </div>
                <div class="pay_type">订购数量：{{data.quantity}}</div>
              </van-col>
              <van-col span="4">
                <el-button type="primary" size="mini" @click="submit(data.kzpurchaseoid)">提交</el-button>
                <el-button type="danger" size="mini" @click="cancel(data.kzpurchaseoid)">取消</el-button>
              </van-col>
            </van-row>
          </div>
  ...
  methods: {
    ...
    cancel(id) {
      Dialog.confirm({
        ...
        .then(() => {
          self.$axios
            .get(self.url + "/api/Pruchase/ReachPurchase?id=" + id)
            .then(res => {
              self.refreshData();
